...

{
    ...
    { 
        ...
        int k2_price = 0;                                        
        bool found = false;                                         
        string current_flight_code ;
       

        while(getline (in_file, line) )
        {
            // cout << "DEBUG: Line read from file: " << line << endl; // Debug statement
           ...
           if (current_flight_code == flight_code1)
           {
                ...
                else if (found && line.find("PRICE PER KID(2 TO 12 YEARS): ") !=string :: npos )//extracting the values of price of kids from 2 to 12 years
                {
                string value2 = line.substr(line.find(":") + 2);
                k1_price = stoi (value2);
                }
                else if (found && line.find("PRICE PER KID(UNDER 2 YEARS): ") !=string :: npos )//extracting the values of price of kids under 2 years
                {
...

{
    ...
    { 
        ...
        {
            cout << "DEBUG: Line read from file: " << line << endl; // Debug statement
           if (line.find("FLIGHT_CODE: " + flight_code1) != string ::npos && line.find("----------BUSINESS CLASS----------") != string::npos)
           {
            found = true;
           }
           if (found)
