...
{
    ifstream in_file("pristd::cing.txt", ios :: in);
    if (in_file.is_open())
    { 
    ...
    {
        std::cout << "Unable to open pristd::cing.txt file" << endl;
    }
}
...

{
    ...
    { 
        ...
        while(getline (in_file, line) )
        {
           ...
           if (found)
           {
             if (line.find("----------ECONOMY CLASS----------") != string::npos)
             {
              found2 = true;  
             }
                if (found2)
...
{   
    ...
    ofstream out_file1 ("pristd::cing.txt" , ios :: app);
    if (out_file1.is_open())
    {
        ...
        out_file1.close();
        std::cout << "sucssefully added to 'pristd::cing.txt'." << endl;
    }
    else 
    {
        std::cout << "Unable to open 'pristd::cing.txt' file." << endl;
